...
def _conv2d(raw,inData, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    if Hook.hookInited :
        ...
        msnhnet.buildConv2d(str(x._cdata), x.size()[1], weight.size()[2], weight.size()[3], 
                            padding[0], padding[1], stride[0], stride[1], dilation[0], dilation[1], groups, useBias,insize,outshape)
    return x
...
def _max_pool2d(raw,inData, kernel_size, stride=None, padding=0, dilation=1,
    ...
    if Hook.hookInited :
        ...
        msnhnet.buildPooling(str(x._cdata), "MAX", kernel_size, kernel_size, stride, stride, 
                                padding, padding, ceilMode, inshape, outshape)
    return x
...
def _linear(raw,inData, weight, bias=None):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildConnect(str(x._cdata), x.size()[1], useBias, inshape, outshape)
    return x
...
def _batch_norm(raw,inData, running_mean, running_var, weight=None, bias=None,
    ...
    if Hook.hookInited : 
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildBatchNorm(str(x._cdata),inshape,outshape,eps)
    return x
...
def _softmax(raw, inData, dim=None, _stacklevel=3):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildSoftmax(str(x._cdata),inshape,outshape)
    return x
...
def _elu(raw, inData, inplace=False):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"elu", x.shape, x.shape)
    return x
...
def _selu(raw, inData, inplace=False):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"selu", x.shape, x.shape)
    return x
...
def _relu(raw, inData, inplace=False):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"relu", x.shape, x.shape)
    return x
...
def _relu6(raw, inData, inplace=False):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"relu6", x.shape, x.shape)
    return x
...
def _leaky_relu(raw, inData, negative_slope=0.01, inplace=False):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"leaky", x.shape, x.shape ,negative_slope)
    return x
...
def _tanh(raw, inData):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"tanh", x.shape, x.shape)
    return x
...
def _sigmoid(raw, inData):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"sigmoid", x.shape, x.shape)
    return x
...
def _softplus(raw, inData, thresh):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"softplus", x.shape, x.shape , thresh)
    return x
...
def _hardswish(raw, inData):
    ...
    if Hook.hookInited :
        ...
        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata),"hardswish",x.shape, x.shape)
    return x

def _silu(raw, inData,inplace=True):
    x = raw(inData,inplace)
    if Hook.hookInited :
        log( "silu-i" , inData._cdata)
        ccc.append(x)
        log( "silu-o" , x._cdata)

        msnhnet.checkInput(inData,sys._getframe().f_code.co_name)
        msnhnet.buildActivation(str(x._cdata) ,"silu", x.shape, x.shape)
    return x
